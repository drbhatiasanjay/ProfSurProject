...
# All torch-dependent code guarded — only defined when PyTorch is installed
if HAS_TORCH:
    ...
    def run_full_forecast(df, features=None, seq_len=5, model_type="LSTM",
        ...
        return {
            "model": model, "model_type": model_type, "features": features,
            "seq_len": seq_len, "test_metrics": test_metrics, "naive_metrics": naive_metrics,
            "train_losses": train_losses, "val_losses": val_losses, "best_epoch": best_epoch,
            "test_preds": test_preds, "test_actuals": split["y_test"],
            "test_years": split.get("years_test", []),
            "n_train": len(split["X_train"]), "n_val": len(split["X_val"]),
            "n_test": len(split["X_test"]),
        }

else:
    # Stubs when PyTorch is not installed
    def run_full_forecast(*args, **kwargs):
        return {"error": "PyTorch is not installed. Run: pip install torch"}

    def forecast_firm(*args, **kwargs):
        return []
